...
The investor deck consists of 15 slides in the following order:

| Slide | ID | Title |
| --- | --- | --- |
| 1 | `cover` | Cover |
| 2 | `mission` | Our Mission |
| 3 | `the-need` | The Need — Who We Serve & Why the Market Fails |
| 4a | `aha-moment` | The Aha Moment |
| 4b | `solution` | Solution — How Cakewalk Works |
| 5 | `tam` | Total Addressable Market — A Massive, Underserved Market |
| 6 | `customer-journey` | Customer Journey — The Transformation at Every Stage |
| 7 | `gtm` | Go-To-Market — Unlock Velocity, Minimize CAC |
| 8 | `competitive-advantage` | Competitive Advantage — Why This Can't Be Replicated |
| 9 | `team` | Team — Built to Serve Customers Others Can't |
| 10 | `board-advisors` | Board & Advisors — Guidance from Industry Leaders |
| 11 | `unit-economics` | Unit Economics — A Profitable Model at Scale |
| 12 | `milestones` | Milestones — Traction and Pathway |
| 13 | `the-ask` | The Ask — Funding Request |
| 14 | `conclusion` | Conclusion — The New Standard |
...
## Slide Format & Aspect Ratio

### 16:9 Standard Slide Format
All slides must use the **16:9 (widescreen)** aspect ratio to ensure consistent presentation formatting across devices and projectors.

**Implementation:**
- Use the `SlideContainer` component (`src/components/ui/SlideContainer.tsx`) as the wrapper for all slide content
- Base resolution: 1920×1080 (Full HD) - design content for this canvas
- The container automatically scales proportionally on different screen sizes
- Maximum width: 1920px; Maximum height: `calc(100vh - 4rem)`

**SlideContainer Props:**
| Prop | Type | Default | Description |
| --- | --- | --- | --- |
| `background` | string | `"bg-white"` | Background color/gradient class |
| `showLogo` | boolean | `true` | Display bottom corner logo |
| `showNavArrow` | boolean | `true` | Display navigation arrow |
| `onNavigateNext` | function | - | Navigation callback |
...
```tsx
<SlideContainer
  background="bg-gradient-to-br from-brand-blue to-brand-purple"
...
**Content Layout within Slides:**
- Use absolute positioning or flexbox within the slide for precise layouts
- Maintain 48-64px padding from slide edges for safe area
- Bottom 64px is reserved for logo and navigation controls
- Design for the 16:9 canvas - avoid content that relies on scrolling

## Layout & Spacing
- Slides use the `SlideContainer` component with 16:9 aspect ratio constraint.
- Primary container: Tailwind `container` (centered, padding 2rem, max width 1400px at `2xl`). Supplement with `px-4` (`16px`) on mobile.
- Section padding: `py-8` to `py-12` within slides to maximize content area while maintaining breathing room.
- Grid defaults:
...
- Maintain consistent vertical rhythm: stack major modules with `mt-8–12`.
- Navigation arrow sits in the bottom-right corner within the slide frame.
